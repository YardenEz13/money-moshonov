...
export async function loadAll() {
  ...
  return {
    tx: tx.map(r => ({ id: r.id, d: r.d, m: r.merchant, note: r.note, a: r.amount, k: r.kind, c: r.category })),
...
export async function saveEntries(items, facts = [], source = "הקלטה", uid) {
  ...
  for (const it of items) {
    if (it.type === "task") {
      if (it.title?.trim()) rows.tasks.push({ user_id: userId, title: it.title.trim(), d: it.date });
    } else if (it.type === "journal") {
      if (it.body?.trim()) rows.jots.push({ user_id: userId, body: it.body.trim(), d: it.date });
    } else {
      rows.transactions.push({
        user_id: userId, d: it.date, merchant: it.merchant?.trim() || "—", note: it.note?.trim() || null,
  ...
  for (const [table, data] of Object.entries(rows)) {
    ...
    const out = must(await sb.from(table).insert(data).select("id"));
...
export async function rotateShortcutToken() {
  ...
  const sb = admin();
  must(await sb.from("shortcut_tokens").delete().eq("user_id", userId));
  must(await sb.from("shortcut_tokens").insert({ token_hash: sha256(raw), user_id: userId }));
  return raw;
}
