...
async function generate({ system, parts, schema, plan, budgetMs = BUDGET_MS }) {
...
async function capture(parts, today, memories) {
  ...
  const lite = await generate({ system, parts, schema: CAPTURE_SCHEMA, plan: "lite" });
  ...
  try {
    const heavy = await generate({
      system, parts, schema: CAPTURE_SCHEMA, plan: "escalate",
