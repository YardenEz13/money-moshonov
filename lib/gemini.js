...
// While other models are still queued, a slow one gets cut off: production logs showed a
// 25s timeout and a 14s 503 before a healthy model answered in 3–5s. The last candidate
// gets whatever budget is left. Import pages legitimately take longer than a spoken entry.
const ATTEMPT_MS = { capture: 12_000, import: 20_000 };

// Set once Gemini rejects thinkingConfig, so later calls on this instance stop sending it.
...
async function generate({ system, parts, schema, tier, budgetMs = BUDGET_MS, maxOutputTokens, attemptMs = ATTEMPT_MS.capture, thinkingLevel }) {
  ...
  const plainBody = makeBody(false);

  const t0 = Date.now();
  const tries = [];
  let res, model, lastError;
  // every model is tried at most once per call: another model beats retrying one that just refused
  const order = candidates(tier);
  for (const [i, m] of order.entries()) {
    const left = budgetMs - (Date.now() - t0);
    if (left < 3000) break;
    model = m;
    const t = Date.now();
    const timeout = () => AbortSignal.timeout(i === order.length - 1 ? left : Math.min(attemptMs, left));
    const send = (body) => fetch(ENDPOINT(m), {
      method: "POST",
    ...
    try {
      const withThinking = !!thinkingLevel && !thinkingRejected;
      res = await send(withThinking ? makeBody(true) : plainBody);
      // the REST field for thinking level isn't pinned in the docs; if this model rejects it,
      // retry right away without it instead of failing every model with the same 400
      if (withThinking && res.status === 400) {
        const peek = await res.clone().json().catch(() => null);
        if (/thinking/i.test(peek?.error?.message || "")) {
          thinkingRejected = true;
          tries.push({ model: m, status: "400-thinking", ms: Date.now() - t });
          res = await send(plainBody);
        }
      }
      ...
      // 503 "high demand" lasted all day on 3.8/3.7 in production; a short bench just re-hit them
      else if (RETRYABLE.has(res.status)) bench(m, 60_000);
  ...
  // every model timed out: for an import chunk the fix is a smaller chunk, so say so
  if (!res) throw fail("Gemini לא ענה בזמן. נסה שוב.", { tries, tooSlow: tries.some(t => t.status === "timeout") });
  if (!res.ok) {
...
function importInstructions({ cardsToo, knownMerchants }) {
  return [
    ...
    "- עמודות: חובה = out, זכות = in. כשיש עמודת סכום אחת: מינוס = out, פלוס = in.",
    "  עמודת יתרה היא מצב החשבון אחרי התנועה — לעולם לא הסכום של התנועה. אם אינך בטוח איזו עמודה היא הסכום, בדוק מול כותרות הקובץ.",
    ...
    "- kind=skip עם skipReason קצר רק כשהשורה אינה הוצאה או הכנסה אמיתית. אלה הסיבות היחידות:",
    "  * העברה בין חשבונות של המשתמש עצמו, הפקדה לחיסכון או לפיקדון שלו.",
    cardsToo
    ...
    "- לעולם לא skip: עמלות בנק או כרטיס, ריבית, דמי ניהול (kind=out, category עמלות);",
    "  העברות לאנשים אחרים או מהם (ביט, פייבוקס, העברה בנקאית) — הוצאה או הכנסה אמיתית;",
    "  זיכוי שמבטל חיוב (החזר מעסק) — kind=in עם שם העסק.",
...
export async function importChunk({ text, pdf, context, cardsToo = true, knownMerchants = [], budgetMs }) {
  ...
  const { out, meta } = await generate({
    ...
    parts, schema: IMPORT_SCHEMA, tier: "heavy", budgetMs, attemptMs: ATTEMPT_MS.import,
    // copying a table into rows isn't a reasoning task; flash models default to high thinking,
    // which took an 80-line chunk past 30s on every model in production
    thinkingLevel: "low",
  });
