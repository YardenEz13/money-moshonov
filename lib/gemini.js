...
const ATTEMPT_MS = { capture: 12_000, import: 30_000 };
...
async function generate({ system, parts, schema, tier, budgetMs = BUDGET_MS, maxOutputTokens, attemptMs = ATTEMPT_MS.capture }) {
  ...
  for (const [i, m] of order.entries()) {
    ...
    try {
      res = await fetch(ENDPOINT(m), {
        ...
        body,
        signal: AbortSignal.timeout(i === order.length - 1 ? left : Math.min(attemptMs, left)),
      });
