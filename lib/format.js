...
// "אחר" stays last: it is the fallback everywhere. The bills/subscriptions/insurance/fees
// buckets exist because a real year of statements otherwise collapses into "אחר".
export const CATS = ["אוכל בחוץ","סופר","תחבורה","דיור","חשבונות","מנויים","ביטוח","בריאות","ביגוד","בידור","עמלות","אחר"];
...
/* ---- diagnostic lines shown as toasts ---- */

const shortModel = (m) => String(m || "?").replace(/^gemini-/, "");
// "3.8-flash:429 → 3.6-flash:timeout → 3.5-flash:200", wrapped in an LTR isolate: inside a Hebrew
// line the leading "3.8" otherwise detaches from "-flash" and jumps to the far end.
export const triesText = (tries = []) =>
  "⁦" + tries.map((t) => `${shortModel(t.model)}:${t.status}`).join(" → ") + "⁩";
