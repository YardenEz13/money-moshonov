...
// A merchant charged about once a month, in 3+ different months, at a steady amount (within 20%
// of its median) is a recurring charge: rent, subscriptions, insurance. The once-a-month part
// keeps weekly supermarket runs of similar size from being called fixed charges.
export function markRecurring(rows) {
  ...
  for (const idx of Object.values(byMerchant)) {
    ...
    const months = new Set(steady.map((i) => rows[i].date.slice(0, 7))).size;
    if (months >= 3 && steady.length <= months * 1.5) steady.forEach((i) => recurring.add(i));
  }
